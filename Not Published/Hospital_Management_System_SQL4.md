...
## Summary
A critical SQL Injection (SQLi) vulnerability exists within the admin-panel.php script. The application fails to properly sanitize or parameterize user-supplied input from the "fname" and "lname" parameter before incorporating it directly into a dynamic SQL query.
...
The vulnerability is triggered by manipulating the fname and lname parameters. Since the input is not escaped or type-casted, an attacker can append SQL control characters to break out of the string context.
